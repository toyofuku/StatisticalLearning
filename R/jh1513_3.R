...
plot(Err0,Err1,col='blue',type='l',ylim=c(0,2.0),xlab="",ylab="")
par(new=T)
plot(Err0,Err2,col='red',type='l',ylim=c(0,2.0),xlab="",ylab="")
...
######################## Neural Network Calculation ##########
###################### Training Initialization ########
w0  <- 0.01*matrix(rnorm(H0*H1),H0,H1)
w1  <- 0.01*matrix(rnorm(H1*H2),H1,H2)
### trained w2 in bottle-neck is used
###  trained w3 in bottle-neck is used
th0 <- 0.01*matrix(rnorm(N),N,1)
